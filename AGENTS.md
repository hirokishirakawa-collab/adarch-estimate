...
- 2026-09-19 14:48 src/lib/navigation/{items.json,catalog.ts}・tests/workspace/invariants.test.tsx・src/app/api/signage/schedules/{_body.ts,route.ts,[id]/route.ts}・scripts/test-workspace.cjs・package.json／前から残っていたテスト2失敗を原因側で修正。(1) /dashboard/leads/tvcm-pool を「本部からの候補先」に改名した際に旧名がaliasesに残っておらず旧名で検索できなくなっていた→旧名を追加し、あわせて検索順を「今の画面名で当たる>旧名だけで当たる」に整理。(2) 拠点スコープ修正(2026-09-17)で入った db.user.findUnique がテストの擬似DBに無く資料検索テストが落ちていた→擬似DBに追加し、Wikiが自拠点＋本部に絞られることを検証する判定を追加。記録にあった型エラー（signage/schedulesのparseScheduleBody）はこのコミットでは再現しなかったが、route.ts はNextの決まった名前しかexportできないため _body.ts へ切り出し。テストの後片付け漏れ（.workspace-tests.css）と npm test の追加。テストは23通過/0失敗（変更前21通過/2失敗）、型0件・ESLint 207件（87 error/120 warning・変更前後同一）・本番ビルド成功。DB・認証・権限・画面の表示内容は変更なし／本番反映していない

- 2026-09-19 14:56 AGENTS.md（前行の本番反映の記録）／代表の「マージ可」を受けPR #3 を squash で main へマージ、9e43147a。内容は前行のとおり（旧名「TVer広告 案件プール」の検索復活・検索順の整理・資料検索テストの擬似DB補完・parseScheduleBody を schedules/_body.ts へ切り出し・テストの後片付けと npm test）。DB変更・マイグレーションなし、認証/権限/画面の表示内容は変更なし。マージ前に main が 59b9c274 から動いていないことを確認。Railwayへは接続していないため、デプロイ結果は代表にご確認いただく／本番反映した（9e43147a・Railwayの完了確認は代表）
