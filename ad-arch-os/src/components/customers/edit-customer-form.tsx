...
// EditCustomerForm
// ---------------------------------------------------------------
export function EditCustomerForm({ customer, staffName }: Props) {
  // updateCustomer は (customerId, prevState, formData) なので bind で customerId を固定
  const boundAction = updateCustomer.bind(null, customer.id);
  const [state, formAction, isPending] = useActionState(boundAction, null);
  const [rank, setRank] = useState<string>(customer.rank);
  ...
  return (
    <form action={formAction} className="space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <Field label="都道府県">
            <div className="relative">
              <select
                name="prefecture"
                defaultValue={customer.prefecture ?? ""}
                className={selectClass}
              >
                <option value="">選択してください</option>
      ...
      <div className="space-y-5">
        ...
        <Field label="担当者" hint="ログインアカウントから自動取得（変更不可）">
