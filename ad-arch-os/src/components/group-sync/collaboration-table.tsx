...
export function CollaborationTable({ requests, role }: Props) {
  const isAdmin = role === "ADMIN";

  // サマリーカウント
  const counts = COLLABORATION_STATUS_OPTIONS.map((opt) => ({
    ...opt,
                ...
                return (
                  <tr key={req.id} className="hover:bg-zinc-50/50 transition-colors group">
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      <Link
                        href={`/dashboard/group-sync/${req.id}`}
                        className="text-sm font-semibold text-zinc-800 hover:text-violet-700 hover:underline"
                      >
                    ...
                    {/* 希望日 */}
                    <td className="px-4 py-3 whitespace-nowrap">
                      <span className="text-xs text-zinc-500">{fmtDate(req.desiredDate)}</span>
                    </td>
