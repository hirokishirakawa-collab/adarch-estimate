...
import { STUDIO_TOOLS, STUDIO_INSTRUCTIONS, STUDIO_CONSULT_PROMPT, publicText } from "../../src/lib/studio/tools";
...
test("公開MCPのツールは決まった7本だけ", () => {
  assert.deepEqual(
    ...
    ["find_ad_awards", "find_subsidies", "list_services", "media_specs", "production_guide", "request_order", "tver_area_plan"],
  );
...
test("値段を返さない：公開する文章から金額・卸・社内向けの行が落ちる", () => {
  const src = ["採用動画は60秒から。", "制作費は1本 30万円〜", "月額 ¥66,000", "単価 6.6円/再生", "卸値は別資料", "ロイヤリティの扱い", "縦型は9:16・1080×1920", "USD $500", "料金はお問い合わせ"].join("\n");
