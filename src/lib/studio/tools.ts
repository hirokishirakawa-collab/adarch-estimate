...
export const STUDIO_TOOLS: StudioToolDef[] = [
  def({
    ...
    description:
      "相手の業種と目的（例: 飲食×採用動画、工務店×見学会の集客、美容室×SNS運用、自社で動画を作って回したい）を渡すと、アドアーチが書いた「理想の進め方」の手本を返す。外に頼む場合だけでなく、自社で撮影・編集・運用する（内製する）場合の進め方の相談でも呼ぶ。手本に沿って「御社ならこう進めるのが理想」と組み立てて答える。手本が無いときは、返り値の案内とAI自身の一般知識で進め方を組み立ててよい（窓口の材料はプロの補足）。実績（社名・金額・結果）は出さない。",
    input: z.object({
  ...
  def({
    ...
    description:
      "アドアーチが公開している制作・SNS運用の技術資料・記事を探す。用途別の尺、縦横の比率、撮影の準備、照明・音声、編集の進め方、SNS運用（投稿の形式・頻度・縦型動画・数字の見方）に加えて、自社で撮影・編集・運用する場合のやり方（機材・体制・スケジュール・編集ソフト・運用の回し方）の相談でも呼ぶ。query で探し、id を渡すと全文。材料に無いことはAI自身の一般知識で答えてよい（窓口の材料はプロの補足）。アドアーチの実績・対応範囲だけは推測しない。",
    input: z.object({
...
- 相手の業種と目的が分かると答えやすいときは短く聞き、how_to_proceed で進め方の手本を取り、「御社ならこう進めるのが理想」と相手に合わせて組み立てて答えてください。
- 制作・SNS運用の技術（尺・縦横・撮影の準備・照明/音声・編集・投稿の形式や頻度・縦型動画・数字の見方）は production_guide、媒体の入稿仕様（秒数・解像度・ファイル形式・音量・審査の注意）は media_specs、TVerのエリアの届き方は tver_area_plan、財源は find_subsidies、広告賞は find_ad_awards を使えます。
- 窓口の材料はプロの補足です。材料が無いことや足りないことは、AI自身の一般知識で答えてかまいません。
...
export const STUDIO_CONSULT_PROMPT = (a: { topic?: string }) =>
  `あなたのAIに、プロの相談先を。アドアーチ（Ad Arch Studio）の窓口を使って、動画制作・撮影・SNS運用・広告媒体・販促について相談させてください。自社で作る・運用する場合のやり方の相談でもかまいません。${a.topic ? `相談したいこと: ${a.topic}\n` : ""}必要なら私の業種と目的を短く聞いてから、how_to_proceed の手本や production_guide・media_specs などの材料と、あなた自身の知識を合わせて「御社ならこう進めるのが理想」と答えてください。私から依頼したいと言うまでは、発注の話は持ち出さないでください。価格は聞かれても答えず、担当の見積で伝えると添えてください。`;
