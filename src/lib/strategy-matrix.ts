...
export const MEDIA_MATRIX: Record<MediaId, MediaDef> = {
  tver: {
    ...
    minBudget: 30_000, // 市町村プランの最低料金（人口5万人未満のエリア）。正本は lib/tver/plan.ts cityPlanTerms（クライアントで読むため数字で持つ）
    recommendedBudget: 1_000_000,
    targetFit: {
      male: 4,
      female: 4,
    ...
    description:
      "民放公式テレビ配信サービス（月間4,470万ユーザー）。売り方は2つ。①市町村プラン（既製の型・Webで申込可）＝1つの市区町村に15秒CM。月額は市の人口で決まる3プラン（住民の200／50／20人に1人へ）で、最低料金は人口5万人未満のエリア月3万円（6ヶ月以上）・5万人以上月5万円（3ヶ月以上）。初回登録費・管理費なし・途中変更なし・配信終了後に結果報告。②大規模展開（オーダー）＝月額30万円以上・2エリア以上・週次報告の希望のどれか。設計・考査費15万円（初回）＋運用管理費（媒体費の20%・最低月5万円）。再生数・届く人数は目安で、お約束しない。具体的な額はOSのTVerシミュレーター・TVer申込リンクで出す。",
    strengths: [
      "市区町村単位で配信でき、小さな市町村なら月3万円から始められる（初回登録費・管理費なし）",
      "広告スキップ不可＝完全視聴率94%台（本部実配信実績）",
      "半分以上がテレビ画面（コネクテッドTV）で視聴される",
      "リーチ人数・フリークエンシーが数字で出る（効果測定可能）",
    ],
    considerations: [
      "最低料金は月3万円〜（人口5万人未満のエリア・6ヶ月以上）／月5万円〜（5万人以上・3ヶ月以上）",
      "15秒の動画素材が必要（大規模展開は30秒・60秒も可）",
      "提案時は再生数でなく「届く人数（目安）」で話す。数字はお約束しない",
      "大規模展開の手数料（設計・考査費・運用管理費）は値引きしない",
    ],
