...
export const MEDIA_MATRIX: Record<MediaId, MediaDef> = {
  tver: {
    ...
    description:
      "民放公式テレビ配信サービス（月間4,470万ユーザー）。売り方の主力は市単位の「商圏網羅プラン」＝商圏のTVer視聴者の3人に1人へ月平均約5回届ける水準（推計値・保証値ではない）。市まるごとで月額24万円〜（例: 高松市58万円/月）、県まるごとで月額72万円〜。標準プランはスターター50万/スタンダード100万（推奨）/ドミナント200万（各/月・媒体費）。期間の考え方: 単月で終わらせず**標準3ヶ月のキャンペーンとして完結**させる＝「商圏の3人に1人へ届く状態」を3ヶ月続けて商圏での認知を取り切る設計。提案金額は月額と3ヶ月総額を併記する。",
    strengths: [
      "市区町村単位で商圏を「まるごと押さえる」提案ができる（地域企業ほど割安）",
      "広告スキップ不可＝完全視聴率94%台（本部実配信実績）",
      "半分以上がテレビ画面（コネクテッドTV）で視聴される",
      "リーチ人数・フリークエンシーが数字で出る（効果測定可能）",
    ],
    considerations: [
      "最低予算50万円〜（スターター・市区町村1つ）",
      "15秒 or 30秒の動画素材が必要",
      "提案時は再生数でなく「リーチ人数」で話す（同じ人に平均約5回当たるため）",
      "単発1ヶ月で効果を断定しない。標準は3ヶ月継続で提案し、見積は月額と3ヶ月総額を併記する",
    ],
