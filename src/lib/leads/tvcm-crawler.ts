    ...
    input_schema: {
      ...
      properties: {
        isVideoAnnouncement: {
          type: "boolean",
          description:
            "候補リストに残すかの判定。基本は true（デフォルト true）。false にしてよいのは次の明確なケースだけ: ①個人投稿のVlog/MV/ゲーム実況/ペット動画、②芸能人/タレント/YouTuberの個人名チャンネル、③第三者（評論家・解説者）による企業批評・レビュー動画、④新聞社/テレビ局/ニュースメディアの報道動画。それ以外は全部 true。営業担当が見て即「これは違う」と分かるものだけ false。少しでも判断に迷ったら true。",
        },
        companyName: {
          type: "string",
...
【超最重要 — isVideoAnnouncement のデフォルトは true】
このシステムは「代表＋グループ加盟者が候補を見て判断」する配布モデル。
あなたはゲートキーパーではない。**基本は全部 true にして候補に残す**。
判断に迷ったら true。営業担当が「これは違う」と即座に判断できる明らかなノイズだけ false。

【false にしてよい4つのケース（これら以外は全部 true）】
① 個人投稿の Vlog / MV 単体 / ゲーム実況 / ペット動画 等、明らかに個人発信
② 芸能人/タレント/YouTuber の個人名チャンネル
③ 第三者（評論家・解説者・YouTuber）による企業や商品の評論・レビュー動画
④ 新聞社・テレビ局・ニュースメディアの報道動画

上記4つ以外は **全部 true**。具体例:
- 法人・店舗・事業者が動画を公開した → true
- 商品発表のついでに動画が添付された記事 → true
- 採用ページに動画が埋め込まれた発表 → true
- 自治体・観光協会・業界団体のPR動画 → true
- 製造業の工場紹介動画 → true
- 建設会社の施工事例動画 → true
- 飲食店のメニュー紹介動画 → true
- 上場企業の大型ブランディング動画 → true（警告バッジで別途識別）
- 大手代理店制作と思われる動画 → true（警告バッジで別途識別）
- 業種が判定不能だがチャンネルが法人っぽい → true
