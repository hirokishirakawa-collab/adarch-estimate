// ==============================================================
// リード獲得AIの採点パイプライン（2026-09-09 に /api/leads/score から切り出し・動きは同じ）
//   Webサイト分析 + YouTube分析 + 成功プロファイル + 今日の判定基準 + Claude で企業リストを採点する。
...
export async function runLeadScoring(body: ScoreLeadsInput, branchIds: string[], apiKey: string) {
...
4. 競合優位性（15点）: アドアーチの強み＝①TVer・イオンシネマ・タクシー広告等の広告媒体の代理店権 ②映像制作、が活きそうか。
   - ★商圏型のBtoC事業（飲食・美容・小売・住宅・医療・教育・レジャー等）で地域の集客ニーズがある → TVer等を「商圏まるごと」提案できる余地が大きく高得点
   - 全国向けEC・BtoB専業など商圏概念が薄い → 媒体の強みは活きにくいが、映像制作・Web広告の提案余地で評価
...
- 合計スコアは各項目の合計（最大100点）
- コメントは営業担当が読む想定で、デジタル活用状況を踏まえた具体的なアプローチのヒントを含める。商圏型のBtoC事業には媒体提案（TVer等の商圏配信）の適性にも触れる
