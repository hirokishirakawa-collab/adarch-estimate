...
const prefsIn = (t: string | null | undefined) => (t ? PREFS.filter((p) => t.replace(/東京都/g, "東京").includes(p)) : []);
...
const AI_TOOL_TEXT: Record<string, (args: Record<string, unknown>) => string> = {
  ...
  create_local_ad: (a) => `AIで${[a.prefecture, a.city].filter(Boolean).join("")}の地域限定広告を設計した`,
  record_local_ad: (a) => `AIで${[a.prefecture, a.city].filter(Boolean).join("")}の地域限定広告を出した`,
  update_local_ad_results: () => "AIでMeta広告の成果を記録した",
