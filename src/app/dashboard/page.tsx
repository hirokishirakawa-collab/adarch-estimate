          ...
          {[
            {
              ...
              desc: "OSにつないだAIに「唐津市の歯科20社にチラシDMを送りたい」と言うと、①日本郵便Webレター用の宛先CSV（そのままアップロードできる形） ②ラクスルDM等に使う汎用の宛名CSV ③貴社名入りのA4チラシPDF（OSのTVerチラシの型・LPや申込ページのQR付き） ④発送先のリンクと手順と概算 を返し、メール・フォームと同じ送付記録（【DM・郵送】）を残します。郵便番号は住所から取り、無ければ補完し、取れない先は「手で補う先」として分けて出します。発送は各社ご自身がWebレターかラクスルDMから行います（本部は送りません・費用は貴社）。画面からも使えます（リード管理の選択バー「郵送DMへ」）。作った材料は「郵送DMの履歴」に残り、再ダウンロードと「発送済み」の記録ができます。チラシは自作のPDFを上げて使うこともできます。これでAI連携の当て方は、メール／フォーム／公式LINE／業種×市のLP／地域限定Meta広告／郵送DM の6つになりました",
              tag: "NEW",
            },
            {
              date: "2026.09.10",
