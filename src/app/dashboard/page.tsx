      ...
      <div className="rounded-xl border border-sky-200 bg-gradient-to-br from-sky-50 via-blue-50 to-indigo-50 px-5 py-4">
        ...
        <div className="space-y-2.5">
          {[
            {
              date: "2026.09.16",
              title: "GROUP LIVEに「顧客登録」と「顧客の活動記録」が流れるように",
              desc: "新しいお客様をOSに登録すると、ACTIVITY FEEDに「顧客登録」として流れます。顧客ページにつけた活動記録（訪問・電話・メール・Web会議）も「活動」として流れます（これまでは商談に紐づくログだけでした）。どちらも社名・業種・都道府県まで／金額・連絡先・メモの中身は出ません。行を押すと、段階・登録日・登録した人・入口・商談の件数が開き、そのまま顧客ページへ移れます。朝のまとめ（Google Chat）にも「新しい顧客」の行が入ります。",
              tag: "機能追加",
            },
            {
              date: "2026.09.15",
              title: "通知の本文をその場で全文読める／AI連携の失敗に理由を添える",
              desc: "右上のベルの通知は、本文があるものを押すとその場で全文が開きます（ページへ移るのは「開く →」から）。AI連携で「保存に失敗しました」「取得に失敗しました」が出たときは、「Googleの検索が一時的に使えませんでした」「採点AIが混み合っていました」など分かる範囲で理由を添えます。少し時間をおいてもう一度お試しください。",
              tag: "改善",
            },
            {
              date: "2026.09.15",
              title: "「送った営業文」— グループの誰かが送ったメール・フォームの文面を、そのまま参考にできる",
              desc: "サイドバー「グループ共有」→「送った営業文」（/dashboard/outreach-messages）。営業フォームやAI連携で「送った」と確定した文面を、同じ日・同じ件名の型ごとにまとめて表示します。訴求の切り口・送った数・返事の内訳（返事待ち・返信あり など）・送った先の業種と地域が並び、本文は宛名を伏せて読めます。文面ごとの「URLをコピー」でチャットにそのまま貼って共有できます。MailSuiteを入れている方は、開封・クリックの通知の件名を貼る（またはAI連携に「MailSuiteの履歴を登録して」と頼む）と、文面ごとに開封・クリックした社数も出ます（開封は目安）。",
              tag: "NEW",
            },
            {
              date: "2026.09.15",
              title: "TVer申込リンクの文面とヘルプAIを「ご相談→Web面談／お電話」の流れに",
              desc: "お客様に送る文面（TVer申込リンク）を「そのままお申込み」から「ご相談ください。Web面談かお電話で内容を確認し、TVerの考査のあと発注書をお送りします」に直しました。ヘルプAIも同じ流れで答えます。市町村プランのような格安プランはWeb面談かお電話で完結するのが理想で、対面は地域の重要企業のみご検討ください。",
              tag: "改善",
            },
            {
              date: "2026.09.15",
              title: "GROUP LIVEの数字を直近3日間に — 前日の受注も、今日の動きも見えるように",
              desc: "上部の数字を、日本時間の今日・昨日・一昨日の3日間にしました。大きな合計の下に「うち今日」を添え、日付が変わっても前日までの成果が見えます。受注はグリーン、反響・アポ獲得・面談予約はゴールドで表示し、地図の選択表示も連動します。",
              tag: "改善",
            },
            {
              date: "2026.09.15",
            ...
            },
            {
              ...
              desc: "ダッシュボードの「今月の活動」の右に、グループ全社の今日の声かけ・商談・受注の件数を並べました（日本時間の0時から・数え方は今月と同じ・件数だけで社名は出ません）。あわせて、今月の受注は受注日で数えるようにし（受注済みの商談にメモを足しただけで今月の受注に入っていたのを修正）、グループライブの「今日」の区切りを日本時間の0時に直しました（これまでは朝9時区切り）",
              tag: "機能追加",
            },
            {
              date: "2026.09.15",
              title: "グループライブに、TVerの動き・営業ツールの利用・お客様からの相談が流れるように",
              desc: "ACTIVITY FEEDに新しく「TVer」（業態考査の申請・OK、配信の申請、配信スタート）と「営業ツール」（郵送DMの用意・発送、営業LP・Web提案書の作成、新しいパッケージの販売開始）が流れます。TVer申込ページからお客様の相談が入ったときも流れます（お客様の名前は出ません）。郵送DM・LP・提案書は市と業種まで、金額はどこにも出ません。入札ファインダーの○判定は1日1行にまとめ、皆さんの動きが埋もれないようにしました",
              tag: "機能追加",
            },
            {
              date: "2026.09.14",
              title: "TVer申込を「相談→面談→業態考査→発注書→お支払い」の流れに変更",
              desc: "申込ページからすぐにお支払いまで進む流れをやめました。お客様はエリア・プラン（目安）とご連絡先を送り、Web面談かお電話を希望します（この段階で料金は発生しません）。案内元の代表（案内元がなければ本部）が内容を確認したあと、本部がTVerの業態考査を申請し、考査が通ってから発注書を発行します。お客様は進捗ページで発注書PDFを確認・署名してからお支払いに進みます。業態考査で出稿不可の場合は発注書を出さないため、返金は起きません。申込規約は v2026-09-15 に更新しています",
              tag: "機能追加",
            },
            {
              date: "2026.09.14",
              title: "グループライブ・朝のまとめ：受注が出直す表示を修正",
              desc: "受注済みの商談にメモや受注の決め手を足すと、その時刻にもう一度「受注」として流れていました。受注は受注日に1回だけ出すようにし、朝のまとめの受注一覧も前日に受注したものだけにしました。あわせて、商談を最初から「受注」で登録したときも受注日が残るようにしました（これまでは後から受注に変えたときだけ）",
              tag: "機能追加",
            },
            {
              date: "2026.09.14",
              title: "AI連携：TVerの申請をメールで出そうとする動きを修正",
              desc: "つないだAIがTVerの配信申請を本部へのメールで出そうとすることがありました。OSからAIへ渡す決まりが長く、Claudeでは途中で切れて申請の流れや記録の決まりが届いていなかったためです。決まりを短くまとめ直し、申請・記録はOSのツールで出すこと、同じ申請が既にあれば出し直さないことを先頭に置きました。今日より前からAIをつないでいる方は、コネクタを一度つなぎ直してください（Claude Codeは起動し直し）",
              tag: "機能追加",
            },
            {
              date: "2026.09.14",
              title: "AI連携から、TVerの業態考査・配信申請まで出せるように",
              desc: "つないだAI（Claude・ChatGPT・アーチくん）に「このお客様でTVerを申請して」と頼むと、広告主の業態考査の状況を確かめ、未申請なら業態考査を申請、承認済みなら組み合わせる市区町村と予算を決めて、エリア一覧・人口・期間・ターゲティングを見せてから配信申請まで出します。チェックと本部への通知は画面から出したときと同じで、承認は本部が行います。申請の詳細画面では、年齢・興味関心・デバイス・フリークエンシーなどの設定も確認できるようになりました",
              tag: "NEW",
            },
            {
              date: "2026.09.14",
              title: "TVer配信申請で、市区町村を複数まとめて選べるように",
              desc: "TVer配信申請の配信エリアで、都道府県に加えて市区町村を指定できるようになりました（TVer広告の市区町村配信エリアと同じ区分）。県をまたいで複数の市を組み合わせたり、政令市を全区まとめて選んだり、県内の市区町村を一括で選べます。あわせて、エリアや年齢などを選ぶと画面が真っ白になる不具合を直しました",
              tag: "機能追加",
            },
            {
              date: "2026.09.14",
              title: "リードを抱えられる件数を100件→200件に",
              desc: "自分が担当で、まだ一度も送っていないリードは200件まで保存できるようになりました（これまでは100件で新しい保存が止まっていました）。リード獲得AI・店舗検索・広告出稿者ファインダー・CSV取込・AI連携の「新しく探す」すべて同じです。取ったまま14日声をかけないリードは、これまで通り自動で担当が外れます",
              tag: "機能追加",
            },
            {
              date: "2026.09.14",
              title: "2つの県を担当している代表は、報告先の県を画面上部で切り替えられます",
              desc: "2拠点をお持ちの代表は、画面上部に「報告先」の切替が出ます。押した県が、これからの商談・送付・リード操作・月次報告・AI連携の記録の登録先になり、グループライブにもその県で流れます。見える範囲は今まで通り両県分です。これまでの登録はそのまま残ります",
              tag: "機能追加",
            },
            {
              date: "2026.09.13",
              title: "TVerの料金が2つの型に — 市町村プランは初回登録費・管理費0円、月3万円から",
              desc: "TVerを「市町村プラン」と「大規模展開」の2つに整理しました。1つの市区町村に15秒で出す市町村プランは、初回登録費・管理費なしで、人口5万人未満のエリアは月3万円の定額プラン（6ヶ月以上）、5万人以上のエリアは人口に応じた3プラン（月5万円から・3ヶ月以上）。お客様はTVer申込リンクからそのまま申込できます。月額30万円以上・2エリア以上・週次報告のご希望は大規模展開として個別に設計します。TVerシミュレーターは市区町村を選ぶだけでどちらのプランかを判定し、手数料まで出します。LP・AI連携・アーチくんの数字もすべて同じ額にそろえました（再生数・届く人数は目安です）",
              tag: "NEW",
            },
            {
              date: "2026.09.13",
              title: "リードを探した直後に、OSが「送っていい先」だけに整えます",
              desc: "リード獲得AI（AI連携の「新しく探す」も同じ）で会社を探すと、その場でOSが各社のサイトを1回見に行き、①メールアドレスを補完 ②「営業お断り」の記載がある会社を自動で対象外にしてグループ共通の送付禁止リストへ登録 ③チェーン・FC・支店（本部決裁で市の商圏の話が通らない先）は最初から保存しない、まで済ませます。一度どなたかが見つけた「お断り」は全拠点で二度と当たりません。あわせて、自拠点が1か月以内に送った先への送り直しはOSが止めます。メールが無い会社は、問い合わせフォームにそのまま貼れる件名・本文と手順が出ます（送信ボタンを押すのは人）。フォームが使えない先は「電話候補」に回すと、ダッシュボードの「今日の一手」に出てきます",
              tag: "NEW",
            },
            {
              date: "2026.09.11",
              title: "受注したら、OSが「おめでとうございます！」— グループの誰かが受注したときもお知らせ",
              desc: "商談を受注にすると、画面の手前で紙吹雪が弾けて「おめでとうございます！」が出ます（クリックで閉じる）。そのまま「受注の決め手を残す」から決め手を書けば、グループ全社が引ける勝ち筋になります。OS画面・スマホ・AI連携のどこで受注にしても、次にOSを開いたときに1回だけ出ます。他の拠点が受注したときは、OSを開いたときに上部へ「◯◯が受注：お客様名（業種）」と小さく出ます（金額は出ません）",
              tag: "NEW",
            },
            {
              date: "2026.09.11",
              title: "AIに「受注した」と話すだけで、受注の確定までOSに残ります",
              desc: "OSにつないだAI（Claude／ChatGPT・右下のアーチくん）に「◯◯社の件、受注しました」と話すと、AIが商談を「受注」に切り替えます。OS画面で受注にしたときと同じく、受注日の記録・プロジェクトの自動作成・本部への受注通知まで動くので、あとから画面で押し直す必要はありません。続けて「決め手は◯◯」と話せば、グループ全社が引ける受注の決め手として残ります。受注済みの商談を戻すときだけはOS画面から。あわせて、画面で受注・失注にしたときも受注日（クローズ日）が記録されるようにしました",
              tag: "NEW",
            },
            {
              date: "2026.09.11",
