...
export default async function AdminTverReportDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const periodBudget = budgetForPeriod(r.monthlyBudget, r.budgetMode, r.periodStart, r.periodEnd); // 媒体実費ベース
        ...
        <div className="lg:col-span-3 space-y-6">
          <section className="bg-white border-2 border-zinc-900 rounded-xl p-5">
            ...
            <div className="grid sm:grid-cols-4 gap-3 text-sm">
              <Stat k={r.budgetMode === "MONTHLY" ? `予算＝媒体実費（${days}日＝${String(billingMonths(r.periodStart, r.periodEnd).toFixed(2)).replace(/\.?0+$/, "")}ヶ月ぶん）` : "予算＝媒体実費（この期間で消化）"} v={periodBudget != null ? yen(periodBudget) : "—"} sub={r.monthlyBudget ? `${r.budgetMode === "MONTHLY" ? `月額 ${yen(r.monthlyBudget)}・` : ""}売価換算 ${yen(periodBudget! * SELL_MULTIPLIER)}` : "未設定"} />
