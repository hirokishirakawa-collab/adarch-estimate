...
function reviewStateOf(t: Template): ReviewState {
  if (t.isApproved) return "APPROVED";
  if (t.rejectedAt) return "REJECTED";
  return "PENDING";
}

const REVIEW_STATE_STYLE: Record<ReviewState, { label: string; badge: string; icon: string; tile: string }> = {
  APPROVED: {
    label: "承認済み",
    badge: "bg-emerald-50 text-emerald-700 border border-emerald-200",
    icon: "text-emerald-600",
    tile: "bg-emerald-50 border border-emerald-200",
  },
  PENDING: {
    label: "承認待ち",
    badge: "bg-amber-50 text-amber-700 border border-amber-200",
    icon: "text-amber-600",
    tile: "bg-amber-50 border border-amber-200",
  },
  REJECTED: {
    label: "差戻し",
    badge: "bg-red-50 text-red-700 border border-red-200",
    icon: "text-red-600",
    tile: "bg-red-50 border border-red-200",
  },
};
...
function TemplateSection({
  ...
  const pendingCount = templates.filter((t) => reviewStateOf(t) === "PENDING").length;
  const rejectedCount = templates.filter((t) => reviewStateOf(t) === "REJECTED").length;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-zinc-200 overflow-hidden">
        ...
        {/* 申請制の状態表示。本部は捌く件数、拠点は自分の申請の状態が一目で分かるようにする。 */}
        {pendingCount > 0 && (
          <div className="px-6 pt-5">
            <div className="rounded-xl bg-amber-50 border border-amber-200 px-4 py-3 flex items-start gap-3">
              ...
              <div className="text-sm text-amber-800 leading-relaxed">
                {isAdmin ? (
                  <>
                    <span className="font-bold">承認待ち {pendingCount}件</span>
                    <span className="ml-2 text-amber-700">
                      承認するまで送信は始まりません。本文を開いて中身を確認してから承認してください。
                    </span>
                  </>
                ) : (
                  <>
                    <span className="font-bold">本部の承認待ち {pendingCount}件</span>
                    <span className="ml-2 text-amber-700">
                      承認されると営業開始で選べるようになります。
                    </span>
                  </>
                )}
              </div>
        ...
        {rejectedCount > 0 && (
          <div className="px-6 pt-3">
            <div className="rounded-xl bg-red-50 border border-red-200 px-4 py-3 text-sm text-red-800 leading-relaxed">
              <span className="font-bold">差戻し {rejectedCount}件</span>
              <span className="ml-2 text-red-700">
                本部の差戻し理由を確認し、修正のうえ再申請してください。
              </span>
            </div>
          </div>
        )}
...
function TemplateCard({ template: t, index, total, readOnly, isAdmin }: { template: Template; index: number; total: number; readOnly?: boolean; isAdmin?: boolean }) {
  ...
  const [dailyLimitInput, setDailyLimitInput] = useState(String(t.dailyLimit ?? 200));
  const [reviewError, setReviewError] = useState("");

  const state = reviewStateOf(t);
  const style = REVIEW_STATE_STYLE[state];

  async function handleTemplatePatch(data: Record<string, unknown>) {
    const res = await fetch(`/api/auto-sales/templates/${t.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(data),
    });
    if (!res.ok) {
      ...
      setReviewError(err.error ?? "更新に失敗しました");
      return;
    }
    window.location.reload();
  }
    ...
    <div className="border border-zinc-200 rounded-xl overflow-hidden hover:border-zinc-300 transition-colors">
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              ...
              {t.isApproved && (
                <span className="text-[10px] px-2 py-0.5 rounded-full font-medium bg-zinc-100 text-zinc-600 border border-zinc-200">
                  上限 {t.dailyLimit}件/日
                </span>
              )}
            </div>
        ...
        {!readOnly && (
          <div className="flex items-center gap-1 pr-4">
            {isAdmin && !t.isApproved && (
              <>
                <button
                  onClick={(e) => { e.stopPropagation(); setApproving(true); setRejecting(false); setExpanded(true); }}
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1 text-xs text-zinc-500 hover:text-red-600 px-2.5 py-2 rounded-lg hover:bg-red-50 transition-colors"
                  title="理由をつけて差し戻す"
                >
                  差戻し
                </button>
              ...
              <button
                onClick={(e) => {
                  ...
                  if (confirm("このテンプレートの承認を取り消しますか？\n営業開始で選択できなくなります。")) {
                    handleTemplatePatch({ reviewAction: "revoke" });
                  }
                }}
            ...
            {!isAdmin && state === "REJECTED" && (
              <button
                onClick={(e) => { e.stopPropagation(); handleTemplatePatch({ resubmit: true }); }}
                className="inline-flex items-center gap-1 text-xs font-bold text-white bg-zinc-900 hover:bg-zinc-800 px-3 py-2 rounded-lg transition-colors"
                title="修正内容で本部に再申請する"
              >
                再申請
              </button>
      ...
      {expanded && (
        <div className="px-4 pb-4 pt-0 border-t border-zinc-100">
          ...
          {/* 返信先。ここに入れたアドレスの受信箱を反響トラッキングが見に行く */}
          <div className={`rounded-xl p-4 mb-3 border ${t.email ? "bg-zinc-50 border-zinc-200" : "bg-amber-50 border-amber-200"}`}>
            <p className="text-xs font-medium text-zinc-500 mb-1">フォームに入れる返信先アドレス</p>
            {t.email ? (
              <p className="text-sm text-zinc-800 font-mono">{t.email}</p>
            ) : (
              <p className="text-sm text-amber-800 leading-relaxed">
                未設定です。このままだと本部の media@adarch.co.jp に届きます。
                拠点の @adarch.co.jp アドレスを設定してください。
              </p>
            )}
          </div>
          ...
          {/* 本部コメント（承認条件・差戻し理由） */}
          {t.reviewNote && (
            <div className={`rounded-xl p-4 mb-3 ${state === "REJECTED" ? "bg-red-50 border border-red-200" : "bg-blue-50 border border-blue-200"}`}>
              <p className={`text-xs font-medium mb-2 ${state === "REJECTED" ? "text-red-700" : "text-blue-700"}`}>
                {state === "REJECTED" ? "本部からの差戻し理由" : "本部コメント"}
              </p>
              <p className={`text-sm whitespace-pre-wrap leading-relaxed ${state === "REJECTED" ? "text-red-800" : "text-blue-800"}`}>
                {t.reviewNote}
              </p>
            </div>
          )}

          {/* 承認パネル（本部のみ）— 送信上限をここで決める */}
          {isAdmin && approving && (
            <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 mb-3">
              <p className="text-xs font-medium text-emerald-800 mb-2">1日の送信上限（このアカウント）</p>
              <div className="flex items-center gap-2">
                <input
                  type="number"
                  min={1}
                  ...
                  className="w-32 text-sm border border-emerald-200 rounded-lg px-3 py-2 bg-white focus:outline-none focus:ring-2 focus:ring-emerald-200"
                />
                <span className="text-xs text-emerald-700">件/日</span>
                {[100, 200, 500].map((n) => (
                  <button
                    key={n}
                    onClick={() => setDailyLimitInput(String(n))}
                    className="text-[11px] text-emerald-700 border border-emerald-200 bg-white px-2 py-1 rounded-md hover:bg-emerald-100 transition-colors"
                  >
                    {n}
                  </button>
                ))}
              ...
              <p className="text-[11px] text-emerald-700 mt-2 leading-relaxed">
                既定は200件/日。上限に達すると、その日はこのアカウントの送信が止まります（翌日 0:00 にリセット）。
                無制限は設定できません（上限 {MAX_DAILY_LIMIT}件/日）。承認後もこの画面から変更できます。
              </p>
              <div className="flex items-center gap-2 mt-3">
                <button
                  ...
                  disabled={
                    !Number.isInteger(Number(dailyLimitInput)) ||
                    Number(dailyLimitInput) < 1 ||
                    Number(dailyLimitInput) > MAX_DAILY_LIMIT
                  }
                  className="text-xs font-bold text-white bg-emerald-600 hover:bg-emerald-700 disabled:opacity-40 px-3 py-2 rounded-lg transition-colors"
                >
                  この条件で承認する
                </button>
                <button
                  onClick={() => { setApproving(false); setReviewError(""); }}
                  className="text-xs text-zinc-500 px-3 py-2 rounded-lg hover:bg-white transition-colors"
                >
                  キャンセル
                </button>
              </div>
          ...
          {isAdmin && rejecting && (
            <div className="bg-red-50 border border-red-200 rounded-xl p-4 mb-3">
              <p className="text-xs font-medium text-red-700 mb-2">差戻しの理由（拠点に表示されます）</p>
              <textarea
                value={rejectNote}
                onChange={(e) => setRejectNote(e.target.value)}
                rows={3}
                placeholder="例: 訴求文に「必ず」「絶対」など断定的な表現があります。表現を修正して再申請してください。"
                className="w-full text-sm border border-red-200 rounded-lg p-3 bg-white focus:outline-none focus:ring-2 focus:ring-red-200"
              />
              <div className="flex items-center gap-2 mt-2">
                <button
                  onClick={() => handleTemplatePatch({ reviewAction: "reject", reviewNote: rejectNote })}
                  disabled={!rejectNote.trim()}
                  className="text-xs font-bold text-white bg-red-600 hover:bg-red-700 disabled:opacity-40 px-3 py-2 rounded-lg transition-colors"
                >
                  差し戻す
                </button>
                <button
                  onClick={() => { setRejecting(false); setRejectNote(""); setReviewError(""); }}
                  className="text-xs text-zinc-500 px-3 py-2 rounded-lg hover:bg-white transition-colors"
                >
                  キャンセル
                </button>
