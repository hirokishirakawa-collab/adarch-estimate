...
export async function POST(
  ...
  try {
    ...
    // Owner check: only proposal owner or ADMIN can publish
    ...
    console.error("[POST /api/proposals/:id/publish]", e);
...
export async function DELETE(
  ...
  try {
    ...
    // Owner check: only proposal owner or ADMIN can unpublish
    if (proposal.userId !== user.id && user.role !== "ADMIN") {
      return NextResponse.json({ error: "Forbidden" }, { status: 403 });
    }
    ...
    console.error("[DELETE /api/proposals/:id/publish]", e);
    return NextResponse.json({ error: "Internal server error" }, { status: 500 });
  }
}
