...
const handler = createMcpHandler(
  (server) => {
    ...
    prompt(
      ...
      { extra: z.string().optional().describe("OSに残っていない今週の声かけ・動き（任意・箇条書きでよい）") },
      (a) =>
        `本部への週次共有を出してください。手順: 1) my_week を1回呼ぶ 2) 返った記録${a.extra ? `と、次の補足「${a.extra}」` : ""}だけから「今週、新しく声をかけた先（件）」「返事があった・会えた（件）」「いちばん受注に近い1件（相手・次の一手・いつまで）」を埋めて見せる 3) 先週の『次の一手』があれば「動きましたか？（やった／途中／やってない）」と私に聞く 4) 「本部に頼みたいこと」を選択肢（新しいプラン／媒体の条件交渉／本部名義で共同提案／他拠点の受注例／値引き・条件の判断／なし）から私に選ばせる 5) 確認が取れたら submit_weekly_share で提出。数字と相手先名は記録にあるものだけ。金額は書かない。`,
    );
    prompt(
