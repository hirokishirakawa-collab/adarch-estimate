...
export async function POST(req: NextRequest) {
  ...
  const limited = checkRateLimit(access.email, "franchise-leads/score", AI_RATE_LIMIT);
...
- グループ参画には加盟金（プランにより120万〜500万円＋税）とロイヤリティ（最低5万円＋案件10% または固定12万円/月の選択制）を無理なく投資できる経済力が必要。**規模の上限は設けない（旧「1〜2人体制まで」は撤廃済み）**。費用を払える経済力があり、成長意欲があり、輩感がなく、自己表現型の作家気質ではない商売気質の事業者。
...
3. 経済力（15点）: ★加盟金（120万〜500万円＋税）を無理なく投資できる事業基盤・資金力があるか。**従業員数の理想・上限は設けない（旧「1〜2人が理想」は撤廃済み）＝規模があること自体は減点しない**。事業年数・店舗数・複数事業・設備投資の形跡などから資金体力を推定して評価。ただし大手チェーン・上場系・全国法人は意思決定構造上対象外として減点。
4. デジタル活用度（15点）: Web/SNS/YouTube活用状況。活用している＝Ad ArchのOS・媒体運用を使いこなせる素地。
5. 独立動機の推定（15点）: 新規収益への意欲＝「探索モード」が立っているか。★FC既加盟・斜陽業種（看板・印刷等の本業縮小）・事業承継の2代目など「新しい事業を探している兆候」は加点。★自社で店舗等を運営する型（お金が社内で回り外部資金が入らない）は減点。儲かりすぎ・完成され尽くした高収益事務所や、安定業態で探索の気配がない先も動機が薄く減点。"他社（顧客）をサポートする立場"で外部資金（顧客の広告予算）を取り込める型を高評価。
