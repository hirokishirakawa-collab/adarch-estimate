...
8. 長さ: 本文は挨拶・署名を除き7〜11行、全体320字程度を目安。要点を絞り、機能やメリットの長い羅列はしない。冒頭は相手の事業に簡潔に触れた一文から入る。
...
この事業者に向けた「Ad Archグループへの参画」のご提案メールの下書きを作成してください。指定された順序（①自己紹介＝広告代理店・制作のグループ／フランチャイズ・グループ形式／なかなか手に入らない媒体代理店権／クリエイティブで全国地域貢献 → ②ご提案趣旨＝双方メリット／**発注ではなく参画前提**を必ず明示 → ③コスト構造＝加盟金とロイヤリティ（最低ロイヤリティあり）／利益率は比較的高い／具体額は書かず個別説明へ → ④URL案内 → ⑤締め）に従い、価格は非開示、褒めない、誇張なしを厳守し、320字程度を目安にしてください。`;
