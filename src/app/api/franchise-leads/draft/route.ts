...
3. **両者メリットを明確に（最重要・必ず両方書く）**:
   ...
   - **広告の"制作（クリエイティブ）"はグループ内部で巻き取れるので、御社は媒体を販売することに集中できる**点（制作の体制やスキルを自前で抱える必要がない＝媒体を"売るだけ"でよい）。←必ず1文入れる
   - 外注経由の中抜きがなく利益率が比較的高い／グループの大手・公的機関の実績を自社営業の提案材料に使える点
   ...
   ※ (A)(B) をそれぞれ簡潔に統合する（箇条書きにしない）。骨子: "御社はいまの事業に媒体販売を足すだけ・制作はグループが請け負う。グループは全国対応体制を完成させたく、まだ業界に存在しない一番手の位置を一緒に取りに行きたい"
4. コスト構造の明示（金額は書かない）:
...
8. 長さ: 本文は挨拶・署名を除き全体550〜650字を上限の目安に、簡潔にまとめる（長くなりすぎない）。各段落は2〜3文まで。同じ趣旨を言い換えて繰り返さない。冒頭は相手の事業に簡潔に触れた一文から入る。
...
- **提案の本質: 相手の既存事業に「広告媒体の販売」を新サービスとして加える。相手は媒体を"売る"側に回り、広告の制作（クリエイティブ）はグループ内部の制作体制で巻き取る＝相手は制作を自前で抱えなくてよい。**
...
この事業者に向けた「Ad Archグループへの参画」のご提案メールの下書きを作成してください。指定順序（①自己紹介＝広告代理店・制作のグループ／フランチャイズ・グループ形式／なかなか手に入らない媒体代理店権／クリエイティブで全国地域貢献 → ②ご提案趣旨＝**動画制作の発注ではなく、御社の"既存事業"に「広告媒体の販売」を新サービスとして加える提案**であること・参画前提を明示 → ③**両者メリット（必ず両方）**：(A)御社側＝いまの事業に媒体販売を足せる／**広告制作はグループが内製で巻き取るので御社は媒体を"売る"ことに集中できる**／中抜きなく利益率高い／グループ実績を自社営業活用、(B)Ad Arch側＝全国に参画を増やしクライアント要望にグループ全体で応える体制を作りたい・**広告で全国リアル対応の代理店グループはまだ存在せず一番手を狙っている**(早期参画は先行者として地域を取れる位置) → ④コスト構造＝加盟金+ロイヤリティ（最低ロイヤリティあり）／金額は個別説明へ → ⑤URL案内 → ⑥締め）に従い、価格非開示・褒めない・誇張なしを厳守、480字程度を目安。`;
  ...
  const TOOLS = [{
    ...
    input_schema: {
      ...
      properties: {
        ...
        body: { type: "string", description: "メール本文（プレーンテキスト、署名なし。挨拶を除き550〜650字程度に簡潔にまとめる。同じ趣旨の言い換え・冗長な前置きは避ける）" },
      },
