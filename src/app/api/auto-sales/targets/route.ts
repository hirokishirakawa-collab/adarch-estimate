...
export async function POST(req: NextRequest) {
  ...
  const blacklisted = await db.autoSalesBlacklist.findFirst({
    where: { domain: { in: domain ? [rawHost, domain] : [rawHost] } },
  });
  if (blacklisted) {
    return NextResponse.json(
      { error: `この企業 (${rawHost}) はブラックリストに登録されています` },
      { status: 400 }
    );
  }

  // 全社の送信済み台帳と突き合わせ（他拠点が既に送っていれば登録させない）
  if (domain) {
    ...
    if (alreadySent) {
      return NextResponse.json(
        {
          error: `${alreadySent.companyName} には ${alreadySent.branch.name} が ${alreadySent.sentAt.toLocaleDateString("ja-JP")} に送信済みです。グループ全体で同じ会社への二重送信はできません。`,
        },
        { status: 409 }
      );
    }
