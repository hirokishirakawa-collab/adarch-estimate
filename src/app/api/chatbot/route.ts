...
## 2026年8〜9月に追加された機能
- **郵送DM（チラシDM）** (/dashboard/leads/dm・リード管理の選択バー「郵送DMへ」) — 選んだ会社に紙のチラシを送る材料（日本郵便Webレター用の宛先CSV・汎用CSV・貴社名とQR入りのA4チラシPDF・発送先リンクと概算）を1回で作り、送付として記録する。発送はWebレターかラクスルDMで代表が行う（費用は貴社）。AIからは prepare_dm
