...
- **広告出稿者ファインダー** (/dashboard/ad-buyer-finder) — すでに有料媒体（ホットペッパービューティー等）に載っている＝広告費を払っている地元の店を県・市・業種で探して一覧化。掲載ページの根拠リンクつき。選んでそのまま営業フォーム・郵送DMへ。HPBは実質有料掲載のみ＝確度「高」、食べログ等は無料枠があるため「中」
...
## 2026年8〜9月に追加された機能
- **郵送DM（チラシDM）** (/dashboard/leads/dm・リード管理の選択バー「郵送DMへ」) — 選んだ会社に紙のチラシを送る材料（ラクスルDMの宛名テンプレに貼り替える汎用CSV・急ぎ少部数向けの日本郵便Webレター用CSV・貴社名とQR入りのA4たたき台PDF＝情報の整理まで・発送先リンクと概算）を1回で作り、送付として記録する。チラシの仕上げは貴社で。発送は基本ラクスルDM（急ぎ少部数はWebレター）で貴社本人が行う（費用は貴社）。AIからは prepare_dm
...
- **リード管理の「解放中（誰でも取れる）」と取得の蓋** (/dashboard/leads/list) — 担当なしの未着手リードを「送れる／電話のみ／連絡手段なし」に分けて県別に表示し、誰でも取れる。担当を取って14日声かけがないと自動で解放される。自分が担当で未送付のリードが200件以上あると新規の保存は止まる（送ってから取る運用）。電話・メール・サイトが全て無い会社には「連絡手段なし」バッジ
...
- プランは2つ（2026-09-13〜）。境目は「運用の手間」
  - **①市町村プラン（既製の型）**: 1つの市区町村・15秒・途中変更なし・配信終了後に結果報告。**初回登録費・管理費なし・値引きなし**。**人口5万人未満のエリアは「まちのプラン」月3万円の1つだけ（6ヶ月以上）**。5万人以上は人口で決まる3プラン（ライト／スタンダード（おすすめ）／フル・最低月5万円・3ヶ月以上）。TVerの在庫で使い切れなかった分は、返金・追加請求なしで配信期間を延ばして配信する。受注の流れは「お客様がTVer申込ページから『ご相談』を送る（この段階で料金は発生しない）→ 案内元の代表がWeb面談（30分）かお電話（15分）で内容を確認 → 本部がTVerの業態考査を申請 → 考査が通ったら本部が発注書を送る → お客様が発注書に署名・お支払いで契約成立」（案内URLは /dashboard/tver-order-link）。**格安プランはWeb面談かお電話で完結するのが理想。対面は地域の重要企業のみ検討**（工数を極限まで下げるため）
  - **②大規模展開（オーダー）**: 月額30万円以上／2エリア以上／週次報告の希望 のどれかに当てはまったら。15・30・60秒・差し替え可・週1報告。**設計・考査費15万円（初回）＋運用管理費（媒体費の20%・最低月5万円）**。手数料は値引きしない
- 言い方は「月額◯円〜（おすすめ：◯円）」。**再生数・届く人数は目安**で、お約束しない（「保証」という言葉は使わない）
- **再生数ではなく「届く人数（目安）」で話す**。同じ人に月4〜5回当たるため、再生数だけで語ると誤認を生む
- 強み4点: ①広告がスキップできない（完全視聴率94%台・本部実績） ②半分以上がテレビ画面で視聴 ③県・市区町村で配信を絞れる ④誰にどれだけ届いたかが数字で出る
- 具体的な金額は TVerシミュレーター (/dashboard/tver-simulator) か TVer申込リンク (/dashboard/tver-order-link) へ案内する（数字はOSが正本。会話で計算しない）
