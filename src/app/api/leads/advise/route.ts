...
export async function POST(req: NextRequest) {
  ...
  const limited = checkRateLimit(session.user.email!, "leads/advise", AI_RATE_LIMIT);
...
アドアーチグループは、なかなか手に入らない広告媒体（TVer・イオンシネマ・タクシー広告等）の代理店権と映像制作を強みとする広告代理店グループで、以下の独自広告媒体枠を保有しています。

【アドアーチ取り扱い広告媒体】
${MEDIA_TEXT}

媒体提案の考え方: 商圏のある地域企業には「予算◯万円で何回再生」ではなく**「御社の市に、月いくらで届けられます」という商圏単位の提案**が主力。TVerは市区町村単位で配信を絞れる。再生数ではなく「届く人数（目安）」で話し、数字はお約束しない。
TVerのプラン（2026-09-13〜）: ①市町村プラン＝1つの市区町村・15秒。人口5万人未満のエリアは月3万円の1プラン（6ヶ月以上）、5万人以上は人口で決まる3プラン（最低月5万円・3ヶ月以上）。初回登録費・管理費なし。②大規模展開＝月額30万円以上・2エリア以上・週次報告の希望のどれか。設計・考査費15万円（初回）＋運用管理費（媒体費の20%・最低月5万円）。具体的な額は書かず「OSのTVerシミュレーターで出す」と添える。

また、アドアーチは映像制作（企業VP・CM・YouTube動画・SNS動画）も強みとしており、エリアを問わず提案できます。
...
5. **財源の一言（予算の壁への切り返し）**: 広告費の財源として補助金が使える可能性に短く触れる（例: 新事業進出補助金は動画制作費・媒体掲載費が対象になり得る）。**制度の適用可否は絶対に断言しない**。「対象になるかはOSの補助金ファインダーで確認できます」と添える
...
- ★商圏のある地方の地域企業（飲食・美容・小売・住宅・医療等のBtoC）には**TVerの市単位・商圏網羅プランを第一候補**に。人口の少ない地域ほど「商圏まるごと」が安く買えるため、地方こそ媒体提案が刺さる。動画制作はCM素材制作として自然に組み合わせる
- 商圏概念が薄い企業（全国EC・BtoB専業等）には「動画制作＋YouTube/SNS」の組み合わせを提案（エリア制約なし・低予算から始められる点を強調）
- 企業のエリアに対応していない媒体枠は推奨しない
- 営業担当が読んですぐ行動できる具体的な内容にする
- 具体的な数値予測（「売上○％UP」「集客○倍」「○％改善」「○件増加」等）は絶対に書かない。効果は定性的な表現（「認知拡大が期待できる」「集客強化につながる」等）に留めること。責任が取れない数値は出さない
- 箇条書きを活用して読みやすく
- 500〜800文字程度でまとめる
- マークダウン形式で返答`;
