...
排除対象: ${body.excludeIndustries.join("、")}`
  ...
  const mediaSection = body.mediaValue && body.mediaScoringHint
    ? `\n\n【重要：広告媒体の適合度を重視してスコアリング】
今回は「${body.industry.replace(' 向けリード', '')}」という広告媒体に響く企業を探しています。
媒体特性: ${body.mediaScoringHint}

「競合優位性」の採点では、この媒体との相性を最重要視してください：
- この媒体のターゲット層と企業の顧客層が一致 → 高得点
- 企業の課題（認知拡大・集客・採用等）にこの媒体が有効 → 高得点
- 企業規模・予算感がこの媒体に合っている → 加点
- コメントには「この媒体でどうアプローチすべきか」の具体的な提案を含めてください

【広告予算ポテンシャル判定 — 最重要】
広告代理店として最も重要なのは「広告費を支払える企業か」です。以下の基準で厳しくフィルタリングしてください：

＜高予算ポテンシャルのシグナル（加点）＞
- Googleレビュー数が多い（50件以上 = 一定の集客力・事業規模がある）
- 自社Webサイトがしっかり構築されている（投資意識がある）
- 複数店舗/支店展開している（売上規模が一定以上）
- 住所がビル内・商業エリア（テナント料を払える＝一定の売上がある）
- 採用ページがある（人件費を使える＝成長中・一定規模の企業）
- BtoB業種で法人取引がメイン（広告単価への理解がある）
- 既にSNSやWeb広告を運用中（広告投資の実績がある）

＜低予算ポテンシャルのシグナル（大幅減点）＞
- レビューが極端に少ない（10件未満）かつ評価も低い → 集客力・事業規模が小さい
- Webサイトがない or 無料ブログレベル → 広告投資への意識が低い
- 個人事業・1人運営の可能性が高い（住所が住宅地、業態が小規模サービス）
- NPO・ボランティア・宗教法人 → 広告予算がない可能性が高い
- 閉業・休業中（CLOSED_*ステータス）→ スコア0点
- 明らかにローカルの個人商店（八百屋・駄菓子屋等）→ 月額広告予算が確保できない

＜媒体別の最低予算ラインの目安＞
- TVer / イオンシネマ / すかいらーくサイネージ → 月30万円〜：中規模以上の企業が前提
- タクシー広告 / ゴルフカート広告 → 月50万円〜：経営者向けBtoB・富裕層向け企業が前提
- おもチャンネル → 月20万円〜：出張客向けサービス
- SNS広告 / Web広告 / SNS運用 → 月10万円〜：小規模でも検討可だが最低限の事業規模は必要
- 動画制作 / YouTube運用 → 案件30万円〜：投資回収できる売上規模がある企業

上記の予算を払えないと推測される企業は、合計スコアを40点以下にしてください。`
    : "";
...
4. 競合優位性（15点）: ${body.mediaValue ? "この広告媒体との相性。媒体のターゲット層と企業の顧客層の一致度を重視。" : "アドアーチの強み（映像制作・OOH広告）が活きそうか。"}
...
- 合計スコアは各項目の合計（最大100点）
- コメントは営業担当が読む想定で、${body.mediaValue ? "この広告媒体での具体的な提案アプローチを含める" : "デジタル活用状況を踏まえた具体的なアプローチのヒントを含める"}
...
[
  {
    ...
    "comment": "${body.mediaValue ? "TVer広告でエリアターゲティングし、地域の見込み客にリーチする提案が有効。動画未活用のため、CM制作とセットで提案可能。" : "動画未活用・SNSなし。映像制作+YouTube運用の提案が効果的。採用ページあり、採用動画の提案も有力。"}"
  }
]`;
