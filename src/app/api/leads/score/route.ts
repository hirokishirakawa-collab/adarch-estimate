...
【広告予算ポテンシャル — 「規模感」の採点に必ず反映】
広告代理店にとって最も重要なのは「広告費を支払える企業か」です。
「規模感（15点）」の採点時に以下を必ず適用してください：

＜高予算シグナル → 規模感12〜15点＞
- 自社Webサイトがしっかり構築されている（投資意識がある）
- 複数店舗・支店展開（売上規模が一定以上）
- ビル内・商業エリアに所在（テナント料を払える）
- 採用ページがある（成長中・一定規模の企業）
- 既にSNSやWeb広告を運用中（広告投資の実績がある）

＜低予算シグナル → 規模感0〜3点 かつ 合計スコアを必ず40点以下にすること＞
- レビューが10件未満かつ評価も低い
- Webサイトがない、または無料ブログレベル
- 個人事業・1人運営の可能性が高い
- NPO・ボランティア・宗教法人
- 閉業・休業中（CLOSED_*ステータス）→ 合計スコア0点

※ 低予算シグナルに該当する企業は、他の項目が高くても合計40点を超えないよう各項目を調整すること`
    : "";
...
3. 規模感（15点）: 広告予算を出せる企業規模か。複数店舗展開・商業エリア所在・採用ページあり・Webサイト整備済み → 高得点。個人事業・住宅地所在・サイトなし → 低得点。${body.mediaValue ? "※ 媒体検索時は「広告予算ポテンシャル」セクションの基準を厳密に適用すること。" : ""}
