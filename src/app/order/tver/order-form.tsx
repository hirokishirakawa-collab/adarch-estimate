...
export function OrderForm(props: {
  ...
  const onPrefChange = (p: string) => {
    ...
    startLoading(async () => {
      ...
      setCity(first);
      applyEstimate(first ? await getAreaEstimate(p, first) : null);
    });
  };
  const onCityChange = (c: string) => {
    setCity(c);
    startLoading(async () => applyEstimate(await getAreaEstimate(pref, c)));
  };

  const planDef = TVER_ORDER_PLANS.find((p) => p.key === plan)!;
  const selected = est?.byPlan[plan];
  const mediaFee = selected?.mediaFee ?? est?.floor ?? 0;
  const canOrder = !!est && !!selected && !selected.custom && !selected.mergedInto;
  const q = useMemo(() => quote(mediaFee, 0, months), [mediaFee, months]);
  const monthChoices = monthOptionsFor(est?.minMonths ?? 3);
  const openConsult = () => {
    setConsultOpen(true);
        ...
        <main>
          <form id="order-form" action={formAction}>
            ...
            <section className="step" id="step-2">
                  ...
                  if (e.custom) {
                    return (
                      <div className="plan-card" key={e.key}>
                        ...
                        <button type="button" className="text-button" onClick={openConsult} style={{ marginTop: 8 }}>大規模展開で相談する →</button>
                      </div>
                    );
                  }
                  return (
              ...
              {est && !est.orderable && (
                <div className="note" style={{ marginTop: 12 }}>
                  <strong>このエリアは大規模展開のご相談になります</strong>
                  <p>
                    どのプランも月額30万円以上になるため、Webでのお申込みではなく個別に設計します。
                    <button type="button" className="text-button" onClick={openConsult} style={{ marginLeft: 8 }}>相談する →</button>
                  </p>
                </div>
              )}
              ...
              <fieldset className="choices choices-inline">
                ...
                {monthChoices.map((m, i) => (
                  <label key={m.months}>
                    ...
                    {i === 0 && <span className="badge recommended" style={{ marginLeft: 8 }}>おすすめ</span>}
                  </label>
                ))}
              </fieldset>
              <p className="small">
                {monthChoices.find((m) => m.months === months)?.note}。お支払いは月払い（毎月その月分）です。
                {est?.small ? `${cityName}は人口5万人未満のため、契約期間は${est.minMonths}ヶ月以上です。` : ""}
                契約期間中のエリア・プラン・動画の変更はできません。
              </p>
