...
import type { ActivityCounts, ActivityKpi } from "@/lib/kpis/activity";

/**
 * 各ダッシュボードのトップに出す「今月の活動」KPIバー。
 * 声かけ → 商談 → 受注 のファネルを件数で表示。個人名は出さない。
 * 非ADMIN には「本部と共有中」バッジを出し、本部が見ている前提を可視化する。
 * 右側に「今日・グループ全体」を同じ数え方で並べる（2026-09-15）。PCは1行・スマホは2段
 */
function Funnel({ counts }: { counts: ActivityCounts }) {
  const items = [
    { icon: Megaphone, label: "声かけ", value: counts.outreach, color: "text-blue-600" },
    { icon: MessagesSquare, label: "商談", value: counts.deals, color: "text-indigo-600" },
    { icon: Trophy, label: "受注", value: counts.won, color: "text-emerald-600" },
  ];
  return (
...
export function ActivityKpiBar({ kpi }: { kpi: ActivityKpi | null }) {
  ...
  return (
    <div className="flex items-center justify-between gap-x-4 gap-y-2 rounded-xl border border-zinc-200 bg-white px-4 py-2.5 flex-wrap">
      {/* 今月 */}
      ...
      {/* 今日・グループ全体 */}
      <div className="flex items-center gap-x-3 gap-y-1 flex-wrap sm:border-l sm:border-zinc-200 sm:pl-4">
        <span
          className="inline-flex items-center gap-1.5 text-[11px] font-semibold text-zinc-500 flex-shrink-0"
          title="日本時間の0時からの、グループ全社の件数"
        >
          <span className="w-1.5 h-1.5 rounded-full bg-sky-500 animate-pulse" />
          今日・グループ全体
        </span>
        <Funnel counts={kpi.today} />
      </div>
    </div>
  );
}
