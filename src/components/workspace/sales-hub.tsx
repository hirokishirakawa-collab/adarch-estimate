...
import { SALES_TASKS, salesToolsForItems, searchSalesTools, type SalesTaskId, type SalesTool } from "@/lib/workspace/sales-tasks";
import { hubItemIcon } from "./hub-visuals";
import { SalesHubScene } from "./sales-hub-scenes";
import styles from "./sales-hub.module.css";

function SalesTaskCard({ task, selected, controls, onSelect }: {
  task: (typeof SALES_TASKS)[number]; selected: boolean; controls: string; onSelect: () => void;
}) {
  ...
  return <button ref={card} type="button" className="sm-choice" aria-pressed={selected} aria-controls={controls}
    ...
    <SalesHubScene task={task.id} />
    <span className="sm-choice-copy"><strong>{task.id === "prepare" ? <><span>提案・連絡を</span><span>準備する</span></> : task.id === "reply" ? <><span>返事・営業結果を</span><span>記録する</span></> : task.title}</strong><span className="sm-choice-caption">{task.caption}</span></span>
  </button>;
}

function ToolRow({ tool }: { tool: SalesTool }) {
...
function FeaturedTool({ tool }: { tool: SalesTool }) {
  const icon = createElement(hubItemIcon(tool.item.href, tool.item.label), { "aria-hidden": true });
...
export function SalesHub({ items, children }: { items: NavigationItem[]; children?: ReactNode }) {
  const tools = useMemo(() => salesToolsForItems(items), [items]);
  const [active, setActive] = useState<SalesTaskId>("find");
  ...
  const task = SALES_TASKS.find((candidate) => candidate.id === active)!;
  const availableTasks = SALES_TASKS.filter((candidate) => tools.some((tool) => tool.task === candidate.id));
  const visible = useMemo(() => {
    const rows = overview ? searchSalesTools(tools, query) : tools.filter((tool) => tool.task === active);
    return sort === "name" ? [...rows].sort((a, b) => a.title.localeCompare(b.title, "ja")) : rows;
  }, [active, tools, query, sort, overview]);
  ...
  const select = (id: SalesTaskId) => { setActive(id); setQuery(""); setSort("purpose"); setSelection((n) => n + 1); };
  return <div className={styles.hub}><div className="sm-content">
    <header className="sm-heading"><div><p className="sm-eyebrow">YOUR NEXT MOVE</p><h1>顧客・営業<span className="sm-title-dot">.</span></h1><p>次の仕事を、ここから。</p></div>
      <div className="sm-filters"><label className="sm-search"><Search aria-hidden /><input ref={input} type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="使いたい機能を検索" aria-label="顧客・営業の機能を検索" />
    ...
    <div className="sm-choices" role="group" aria-label="進めたい仕事">{availableTasks.map((candidate) => <SalesTaskCard key={candidate.id} task={candidate} selected={!overview && active === candidate.id} controls={panelId} onSelect={() => select(candidate.id)} />)}</div>
    {children}
    <section className="sm-task-detail" aria-labelledby={titleId}>
      ...
      <div ref={panel} id={panelId}>
        {visible.length === 0 ? <p className="sm-empty">該当する機能がありません。別の言葉で検索してください。</p> : overview ? visible.map((tool) => <ToolRow key={tool.item.href} tool={tool} />) : <>
