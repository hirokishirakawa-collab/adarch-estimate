...
export function PublicProposalView({ proposal, preview, onClose }: PublicProposalViewProps) {
  ...
  return (
    ...
    >
      ...
      {/* ━━━ スライドエリア（画面表示用） ━━━ */}
      <div data-print-hide className="flex-1 relative flex items-center justify-center px-6 py-4">
        {/* 左矢印 */}
        {currentSlide > 0 && (
          <button
            onClick={goPrev}
      ...
      {/* ━━━ スライドインジケーター ━━━ */}
      <div data-print-hide className="flex-shrink-0 flex items-center justify-center gap-2 py-3">
...
// ─── 印刷用: 全スライドを16:9横向きでページ分割 ───
function PrintSlides({ content: c, companyName, recipientName, presenterCompany }: {
  content: ProposalContent; companyName: string; recipientName: string; presenterCompany: string;
}) {
  ...
  return (
    <>
      {/* スライド0: 表紙 */}
      <div className="print-slide" style={{ ...slideStyle, background: "#FAFAFA" }}>
        <svg className="absolute inset-0 w-full h-full" preserveAspectRatio="none" viewBox="0 0 960 540">
          <line x1="-20" y1="500" x2="980" y2="-20" stroke="#4285F4" strokeWidth="2.5" opacity="0.18" />
          <line x1="-20" y1="510" x2="980" y2="-10" stroke="#4285F4" strokeWidth="0.8" opacity="0.08" />
          <line x1="80" y1="560" x2="920" y2="20" stroke="#EA4335" strokeWidth="2" opacity="0.14" />
        ...
        <div className="relative z-10 h-full flex flex-col justify-between px-16 py-10">
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-4">
              ...
              <img src="/logo-adarch.png" alt="Ad Arch Group" className="h-8 w-auto" />
              <div className="flex gap-0.5 ml-2">
                <div className="w-1.5 h-6 rounded-sm" style={{ background: "#4285F4" }} />
                <div className="w-1.5 h-6 rounded-sm" style={{ background: "#EA4335" }} />
                <div className="w-1.5 h-6 rounded-sm" style={{ background: "#FBBC04" }} />
            ...
            <div className="text-right">
              <p className="text-xs tracking-wider font-medium" style={{ color: "#4285F4" }}>PREPARED FOR</p>
              <p className="text-sm font-medium mt-1" style={{ color: brand.primary }}>{recipientName}</p>
            </div>
          </div>
          <div className="max-w-lg">
            <div className="flex items-center gap-4 mb-5">
              ...
              <p className="text-xs tracking-widest font-bold" style={{ color: brand.textSub }}>PROPOSAL</p>
            </div>
            <h1 className="text-3xl font-bold leading-tight mb-4" style={{ color: brand.primary, letterSpacing: "0.02em" }}>
              {c.cover.title}
            </h1>
            <p className="text-sm font-light leading-relaxed" style={{ color: brand.textSub }}>{c.cover.subtitle}</p>
          </div>
          <div className="flex items-end justify-between">
            <div>
              <p className="text-xs tracking-wider mb-1" style={{ color: brand.textSub }}>DATE</p>
            ...
            <p className="text-xs" style={{ color: "#B0ADA6" }}>Confidential — {companyName} 様限定</p>
          </div>
        </div>
      </div>

      {/* スライド1: 会社紹介 */}
      <div className="print-slide" style={{ ...slideStyle, background: brand.slideBg }}>
        <ArchLines variant={1} />
        <div className="relative z-10 h-full flex flex-col justify-center px-16 py-10">
          <SlideHeader label={`${companyName} 様への私たちの紹介`} title={c.companyIntro.heading} />
          <p className="text-sm leading-relaxed mb-8" style={{ color: brand.textSub, maxWidth: 600 }}>{c.companyIntro.description}</p>
          <div className="grid grid-cols-3 gap-4">
            {c.companyIntro.strengths.map((s, i) => {
              const colors = ["#4285F4", "#EA4335", "#FBBC04", "#34A853"];
              return (
                <div key={i} className="rounded-lg px-5 py-5 text-center" style={{ background: brand.bg, border: `1px solid ${brand.border}` }}>
                  <div className="w-8 h-0.5 mx-auto mb-3" style={{ background: colors[i % colors.length] }} />
                  <p className="text-sm font-medium" style={{ color: brand.text }}>{s}</p>
                </div>
              );
            })}
      ...
      <div className="print-slide" style={{ ...slideStyle, background: brand.slideBg }}>
        <ArchLines variant={2} />
        <div className="relative z-10 h-full flex flex-col justify-center px-16 py-10">
          <SlideHeader label={`${companyName} 様へのご提案`} title={c.proposal.heading} />
          <div className="rounded-lg px-5 py-4 mb-5" style={{ background: brand.bg, borderLeft: `3px solid ${brand.accent}` }}>
            <p className="text-xs font-medium tracking-wider mb-1" style={{ color: brand.accent }}>{companyName} 様にお力になれること</p>
            <p className="text-sm leading-relaxed" style={{ color: brand.text }}>{c.proposal.challenge}</p>
          </div>
          <div className="space-y-2.5">
            {c.proposal.solutions.map((sol, i) => {
              const colors = ["#4285F4", "#34A853", "#FBBC04", "#EA4335"];
              return (
                <div key={i} className="rounded-lg px-5 py-3.5" style={{ background: brand.bg, border: `1px solid ${brand.border}` }}>
                  <div className="flex items-start gap-3">
                    <span className="flex-shrink-0 w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold" style={{ background: colors[i % colors.length], color: "#FFFFFF" }}>
                      {String(i + 1).padStart(2, "0")}
                    </span>
                    <div>
                      <p className="text-sm font-bold mb-0.5" style={{ color: brand.text }}>{sol.title}</p>
                      <p className="text-xs leading-relaxed" style={{ color: brand.textSub }}>{sol.description}</p>
                    </div>
                  </div>
                </div>
              );
      ...
      <div className="print-slide" style={{ ...slideStyle, background: brand.slideBg }}>
        <ArchLines variant={3} />
        <div className="relative z-10 h-full flex flex-col justify-center px-16 py-10">
          <SlideHeader label={`${companyName} 様と同業界での実績`} title={c.cases.heading} />
          <div className="grid grid-cols-2 gap-4">
            {c.cases.items.map((item, i) => {
              const colors = ["#4285F4", "#34A853", "#FBBC04", "#EA4335"];
              return (
                <div key={i} className="rounded-lg px-5 py-5" style={{ background: brand.bg, border: `1px solid ${brand.border}`, borderTop: `3px solid ${colors[i % colors.length]}` }}>
                  <p className="text-sm font-bold mb-2" style={{ color: brand.text }}>{item.title}</p>
                  <p className="text-xs leading-relaxed" style={{ color: brand.textSub }}>{item.description}</p>
                </div>
              );
            })}
          </div>
      ...
      <div className="print-slide" style={{ ...slideStyle, background: brand.slideBg }}>
        ...
        <div className="relative z-10 h-full flex flex-col items-center justify-center px-16 py-10 text-center">
          <div className="flex items-center gap-3 mb-8">
            {[
              { bg: "#4285F4", d: "M12 20h9M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4Z" },
              { bg: "#EA4335", d: "" },
              { bg: "#FBBC04", d: "" },
              { bg: "#34A853", d: "" },
            ].map((icon, i) => (
              <div key={i} className="w-12 h-12 rounded-xl flex items-center justify-center" style={{ background: icon.bg }}>
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  {i === 0 && <><path d="M12 20h9" /><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4Z" /></>}
                  {i === 1 && <><rect x="3" y="3" width="18" height="18" rx="2" /><circle cx="9" cy="9" r="2" /><path d="m21 15-3.086-3.086a2 2 0 0 0-2.828 0L6 21" /></>}
                  {i === 2 && <><path d="m22 8-6 4 6 4V8Z" /><rect x="2" y="6" width="14" height="12" rx="2" /></>}
                  {i === 3 && <><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z" /><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z" /></>}
                </svg>
              </div>
            ))}
          </div>
          <p className="text-xs tracking-widest font-medium mb-4" style={{ color: brand.textSub }}>CREATIVE PROPOSAL</p>
          <h2 className="text-2xl font-bold leading-tight mb-6" style={{ color: brand.primary, letterSpacing: "0.02em" }}>
            クリエイティブのご提案について
          </h2>
          <p className="text-sm leading-relaxed max-w-md" style={{ color: brand.textSub }}>
            クリエイティブのご提案につきましては<br />
      ...
      <div className="print-slide" style={{ ...slideStyle, background: brand.slideBg }}>
        <ArchLines variant={4} />
        <div className="relative z-10 h-full flex flex-col justify-center px-16 py-10">
          <SlideHeader label={`${companyName} 様とのお取り組みに向けて`} title={c.nextSteps.heading} />
          <div className="flex gap-8">
            <div className="flex-1 space-y-3">
              {c.nextSteps.steps.map((step, i) => {
                const colors = ["#4285F4", "#EA4335", "#FBBC04", "#34A853"];
                return (
                  <div key={i} className="flex items-start gap-3">
                    <div className="flex-shrink-0 w-7 h-7 rounded-full flex items-center justify-center" style={{ background: colors[i % colors.length] }}>
                      <span className="text-xs font-bold" style={{ color: "#FFFFFF" }}>{String(i + 1).padStart(2, "0")}</span>
                    </div>
                    <p className="text-sm leading-relaxed pt-1" style={{ color: brand.text }}>{step}</p>
                  </div>
                );
              })}
            </div>
            <div className="flex-1 rounded-lg px-6 py-6 flex flex-col items-center justify-center text-center" style={{ background: brand.primary }}>
              <div className="flex gap-1 mb-3">
                <div className="w-3 h-0.5 rounded-full" style={{ background: "#4285F4" }} />
                <div className="w-3 h-0.5 rounded-full" style={{ background: "#EA4335" }} />
                <div className="w-3 h-0.5 rounded-full" style={{ background: "#FBBC04" }} />
              ...
              <p className="text-xs tracking-wider mb-2" style={{ color: brand.accent }}>CONTACT</p>
              <p className="text-sm font-medium mb-1" style={{ color: "#FFFFFFEE" }}>{presenterCompany}</p>
              {c.presenter?.name && <p className="text-xs mb-1" style={{ color: "#FFFFFFBB" }}>担当: {c.presenter.name}</p>}
              <p className="text-xs leading-relaxed" style={{ color: "#FFFFFF99" }}>
                {c.presenter?.phone && <>{c.presenter.phone}<br /></>}
                {c.presenter?.email && <>{c.presenter.email}</>}
              </p>
              {c.nextSteps.contact && <p className="text-xs leading-relaxed mt-2" style={{ color: "#FFFFFFAA" }}>{c.nextSteps.contact}</p>}
              <div className="flex gap-1 mt-3">
                <div className="w-3 h-0.5 rounded-full" style={{ background: "#4285F4" }} />
                <div className="w-3 h-0.5 rounded-full" style={{ background: "#EA4335" }} />
                <div className="w-3 h-0.5 rounded-full" style={{ background: "#FBBC04" }} />
