...
import {
  COLLABORATION_REQUEST_TYPE_OPTIONS,
...
export function CollaborationForm({ action, projects }: Props) {
    ...
    <form action={formAction} className="space-y-5 max-w-xl" encType="multipart/form-data">
      ...
      <div>
        <label className="block text-xs font-semibold text-zinc-700 mb-1.5">
          連携先代表（相手）<span className="text-red-500 ml-0.5">*</span>
        </label>
        <input
          type="text" name="counterpartName"
          placeholder="例: XX県 XX様"
      ...
      <div>
        <label className="block text-xs font-semibold text-zinc-700 mb-1.5">
          依頼種別<span className="text-red-500 ml-0.5">*</span>
        </label>
        <select name="requestType" required className={inputCls}>
          <option value="">— 選択してください —</option>
          {COLLABORATION_REQUEST_TYPE_OPTIONS.map((opt) => (
      ...
      <div>
        <label className="block text-xs font-semibold text-zinc-700 mb-1.5">
          依頼内容・条件<span className="text-red-500 ml-0.5">*</span>
        </label>
        <textarea
          name="description" rows={8}
          placeholder="依頼の詳細、条件、注意事項などを記入してください"
          required
